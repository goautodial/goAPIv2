<?php
    ...
    if (!checkIfTenant($groupId)) {
        ...
        $query_GetUserInfo = "SELECT user_id, user, full_name, email, user_group, active, user_level, phone_login, phone_pass, voicemail_id, hotkeys_active, vdc_agent_api_access, agent_choose_ingroups FROM vicidial_users WHERE $ul";
        ...
        $incallsresults = mysqli_fetch_assoc($rsltvInCallsToday);
        $insalesresults = mysqli_fetch_assoc($rsltvInSalesToday);
        $outcallsresults = mysqli_fetch_assoc($rsltvOutCallsToday);
        $outsalesresults = mysqli_fetch_assoc($rsltvOutSalesToday);
        ...
        if ($user != NULL && $query_OnlineAgents != NULL){
            ...
            $data = array_merge($dataInCalls, $dataNoCalls, $dataParkedChannels, $dataCallerIDsFromVAC);  
                ...
                $apiresults = array("result" => "success", "data" => $fresults, "incallstoday" => $incallsresults, "outcallstoday" => $outcallsresults, "insalestoday" => $insalesresults, "outsalestoday" => $outsalesresults);
        }
    }

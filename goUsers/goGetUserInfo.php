	...
	if (empty ($goUser) || is_null ($goUser)) {
		...
		if ($goapiaccess > 0 && $userlevel > 7) {	   
			...
			if ($astDB->count > 0) { // Yes, I am!					
				...
				} elseif (strtoupper((string) $log_group) !== 'ADMIN') {
					if ($userlevel > 8) {
						$astDB->where("user_group", $log_group);
					}
				}				
				...
				if ($astDB->count > 0) { // Yes, you are!
					...
					if ($astDB->count > 0) { // Yes, I am!							
						...
						$outcallstoday					= $astDB
							...
							->where("vl.call_date", ["$datetoday 00:00:00", "$datetoday 23:59:59"], "BETWEEN")
						...
						if (strstr("/READY|CLOSER/", (string) $onlineako["status"])) { // I'm waiting for calls..
							$table						= "
						...
						if (strstr("/PAUSED/", (string) $onlineako["status"])) { // I'm waiting for calls..
							$table						= "
								vicidial_live_agents,
								vicidial_users,
								vicidial_agent_log,
						...
						if (strstr ("/INCALL|QUEUE|PARK|3-WAY/", (string) $onlineako["status"])) { // I'm on a call..
							$table						= "
								vicidial_live_agents,
								vicidial_users,
								vicidial_list,
						...
						$apiresults 					= [// Since I'm offline, here's what you get..
							"result" 						=> "success", 
							"data" 							=> $data,
							"avatar"						=> $avatar
						];					

...
**/
    ...
    if(empty($user)) { 
		...
		$outbound_query = $astDB->rawQueryValue("SELECT uniqueid, lead_id, list_id, campaign_id, call_date, start_epoch, end_epoch, length_in_sec, status, phone_code, phone_number, user, comments, processed, user_group, term_reason, alt_dial FROM vicidial_log WHERE $ul $daterange ORDER BY call_date DESC LIMIT 10000;", $arrul);
		...
		while($outbound_query as $outbound_fetch){
		...
		while($closerlog_query as $closerlog_fetch){
		...
		$userlog_query = $astDB->rawQueryValue("(SELECT agent_log_id, user, sub_status, event_time, campaign_id, user_group FROM vicidial_agent_log WHERE $ul and sub_status IS NOT NULL $daterange1) UNION (SELECT user_log_id, user, event, event_date, campaign_id, user_group FROM vicidial_user_log WHERE $ul $daterange2) ORDER BY event_time DESC;", );
		...
		while($userlog_query as $userlog_fetch)){

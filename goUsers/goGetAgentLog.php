...
**/
	...
	if (empty($goUser) || is_null($goUser)) {
		...
		if ($goapiaccess > 0 && $userlevel > 7) {
			...
			$astDB->where("date_format(call_date, '%Y-%m-%d %H:%i:%s')", [$start_date, $end_date], "BETWEEN");
			...
			$closerlog_query  							= $astDB
				->where("user", $user)
				->where("date_format(call_date, '%Y-%m-%d %H:%i:%s')", [$start_date, $end_date], "BETWEEN")

...
/** @var string $log_ip */
	...
	$user_name 											= isset($_REQUEST['user_name']) ? $astDB->escape($_REQUEST['user_name']) : '';
	$user_email 										= isset($_REQUEST['user_email']) ? $astDB->escape($_REQUEST['user_email']) : '';
	$user_pass 											= isset($_REQUEST['user_pass']) ? (string) $_REQUEST['user_pass'] : '';
	$ip_address 										= isset($_REQUEST['ip_address']) ? $astDB->escape($_REQUEST['ip_address']) : '';
	$auth 												= 0;
	...
	$cols												= ["user_id", "user", "email", "pass", "full_name", "user_level", "user_group", "active", "pass_hash", "phone_login", "phone_pass"];
	if (!empty($user_name)) {
		$astDB->where("user", $user_name);
	} else {
		$astDB->where("email", $user_email);
	}

	$rsltv 											= $astDB->get("vicidial_users", 1, $cols);
	$storedPasswordHash 							= $rsltv[0]['pass_hash'] ?? '';
	$storedPlainPassword 						= $rsltv[0]['pass'] ?? '';
	$passwordValid 								= false;
	error_log($storedPlainPassword);
	if (!empty($storedPasswordHash) && preg_match('/^\$2y\$/', (string) $storedPasswordHash)) {
		$passwordValid = password_verify($user_pass, (string) $storedPasswordHash);
	} elseif ($SSpass_hash_enabled > 0) {
		$passwordValid = false;
	} else {
		$passwordValid = hash_equals((string) $storedPlainPassword, $user_pass);
	}

	if	($rsltv > 0 && $passwordValid) {
		...
		$thisGroup 										= ((string) $rslt['user_group'] !== '') ? $rslt['user_group'] : "";

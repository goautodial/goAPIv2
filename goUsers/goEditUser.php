...
/** @var string $log_ip */
	...
	if (empty ($goUser) || is_null ($goUser)) {
    ...
    } elseif (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $user) || preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $userid)){
	...
	} elseif (preg_match('/[\'^£$%&*()}{@#~?><>|=+¬]/', (string) $full_name)){
		$err_msg 										= error_handle("41006", "full_name");
		$apiresults 									= [
			"code" 											=> "41006",
			"result" 										=> $err_msg
		];
	} elseif (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $phone_login)){
		...
		$updateUserGoArray 								= array_merge($updateUserGoArray, ["location_id" => $location]);
		$insertUserGoArray 								= array_merge($insertUserGoArray, ["location_id" => $location]);
	} else {

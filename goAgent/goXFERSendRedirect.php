...
if ($is_logged_in) {
    ...
    if ($ACTION == "RedirectVD") {
        if ( strlen((string) $channel) < 3 || strlen((string) $queryCID) < 15 || strlen((string) $exten) < 1 || strlen((string) $campaign) < 1 || strlen((string) $ext_context) < 1 || strlen((string) $ext_priority) < 1 || strlen((string) $uniqueid) < 2 || strlen((string) $lead_id) < 1 ) {
            $channel_live = 0;
            $message  = "One of these variables is not valid:\n";
            if (strlen((string) $channel) < 3) {$message .= "'channel' must be greater than 2 characters\n";}
            if (strlen((string) $queryCID) < 15) {$message .= "'queryCID' must be greater than 14 characters\n";}
            if (strlen((string) $exten) < 1) {$message .= "'exten' must be set\n";}
            if (strlen((string) $ext_context) < 1) {$message .= "'ext_context' must be set\n";}
            if (strlen((string) $ext_priority) < 1) {$message .= "'ext_priority' must be set\n";}
            if (strlen((string) $auto_dial_level) < 1) {$message .= "'auto_dial_level' must be set\n";}
            if (strlen((string) $campaign) < 1) {$message .= "'campaign' must be set\n";}
            if (strlen((string) $uniqueid) < 1) {$message .= "'uniqueid' must be set\n";}
            if (strlen((string) $lead_id) < 1) {$message .= "'lead_id' must be set\n";}
    ...
    if ($ACTION == "RedirectToPark") {
        if ( strlen((string) $channel) < 3 || strlen((string) $queryCID) < 15 || strlen((string) $exten) < 1 || strlen((string) $extenName) < 1 || strlen((string) $ext_context) < 1 || strlen((string) $ext_priority) < 1 || strlen((string) $parkedby) < 1 ) {
            ...
            if (strlen((string) $exten) < 1) {$message .= "'exten' must be set\n";}
            if (strlen((string) $ext_context) < 1) {$message .= "'ext_context' must be set\n";}
            if (strlen((string) $extenName) < 1) {$message .= "'extenName' must be set\n";}
            if (strlen((string) $ext_priority) < 1) {$message .= "'ext_priority' must be set\n";}
            if (strlen((string) $parkedby) < 1) {$message .= "'parkedby' must be set\n";}
    ...
    if ($ACTION == "RedirectXtraCXNeW") {
        ...
        $channel_liveX = 1;
        $channel_liveY = 1;
        if ( strlen((string) $channel) < 3 || strlen((string) $queryCID) < 15 || strlen((string) $ext_context) < 1 || (strlen((string) $ext_priority) < 1 || strlen((string) $session_id) < 3 || (strlen((string) $extrachannel) < 3 || strlen((string) $exten) < 1) && !preg_match("/NEXTAVAILABLE/", (string) $exten)) ) {
            $channel_liveX = 0;
            $channel_liveY = 0;
            $message  = "One of these variables is not valid:\n";
            $message .= "Channel must be greater than 2 characters\n";
            ...
            $message = '';
            $result = 'error';
            if (preg_match("/NEXTAVAILABLE/", (string) $exten)) {
                //Confbridge
            ...
            if ($exitThis < 1) {
                ...
                if ( ($channel_liveX === 1) && ($channel_liveY === 1) ) {
                    ...
                    if ($rowx_ct < 1) {
                        ...
                        $D_s_ip = explode('.', (string) $dest_server_ip);
        ...
        if ($channel == "$extrachannel") {
            ...
            if ( (strlen((string) $channel) < 3) || (strlen((string) $queryCID) < 15) || (strlen((string) $ext_context) < 1) || (strlen((string) $ext_priority) < 1) || (strlen((string) $session_id) < 3) || ( ( (strlen((string) $extrachannel) < 3) || (strlen((string) $exten) < 1) ) && (!preg_match("/NEXTAVAILABLE/", (string) $exten)) ) ) {
                ...
                if ($exitThis < 1) {
                    ...
                    $rslt = $astDB->get('live_channels');
                    $row_ct = $astDB->getRowCount();
                    if ( ($row_ct == 0) && (!preg_match("/SECOND/", (string) $filename)) ) {
                        //$stmt="SELECT count(*) FROM live_sip_channels where server_ip = '$call_server_ip' and channel='$channel';";
                        $astDB->where('server_ip', $call_server_ip);
                        $astDB->where('channel', $channel);
                        $rslt = $astDB->get('live_sip_channels');
                    ...
                    if ( ($channel_liveX === 1) && ($channel_liveY === 1) ) {
                        if ( ($server_ip == "$call_server_ip") || (strlen((string) $call_server_ip) < 7) ) {
    ...
    if ($ACTION == "Redirect") {
        ...
        if ($stage == "2NDXfeR") {
            ...
            if ($row_ct > 0) {
                ...
                $channel = preg_replace("/1$/i","2", (string) $channel);

...
if ($sipIsLoggedIn) {
	...
	$channel_live = 1;
	if ( (strlen((string) $stage) < 1) || (strlen((string) $called_count) < 1) || (strlen((string) $lead_id) < 1) ) {
		$channel_live = 0;
		$APIResult = [ "result" => "error", "message" => "LEAD NOT REVERTED" ];
	} else {
		$called_count -= 1;
		...
		$APIResult = [ "result" => "success", "message" => "LEAD REVERTED" ];
	}

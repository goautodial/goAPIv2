...
if (isset($goAction) && $goAction != "") {
    if (preg_match("/$actions/", $goAction)) {
        ...
        if ($auth < 1) {
            ...
            if ($allowedAPIAccess) {
                if (!preg_match("/goGetCallbackCount|goCheckIfLoggedIn|goGetScriptContents|goCheckConference|goGetLoginInfo|goGetAllowedCampaigns|goLogoutUser|goManualDialLookCall|goClearAPIField|goGetLabels|goXFERSendRedirect/", $goAction) && (!isset($campaign) || $campaign == '')) {

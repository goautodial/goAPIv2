...
/** @var string $log_ip */
    ...
    $asterisk_version = is_array($query) ? ($query['asterisk_version'] ?? '') : '';
    $DBSERVER_GMT = is_array($query) ? ($query['local_gmt'] ?? '') : '';
    $gmt_recs = ((string) $DBSERVER_GMT !== '') ? 1 : 0;
    if ($gmt_recs > 0) {
	...
	$goCountTheHeader = "";
	$goGetLastHeader = '';
	$goGetLastCustomFiledsName = '';
	$goGetLastHeader2 = [];
	$goGetLastCustomFiledsName2 = [];
	$col = [];
	$standard_array = [];
	$custom_array = [];
	$Shour = date('H');
	...
	if (($handle = fopen($csv_file, "r")) !== FALSE) {
		$getHeder = fgetcsv($handle, escape: '\\');
		//$goInsertSuccess = 0;
		...
		if($goCountTheHeader > 21 && !empty($lead_mapping)) {
			...
			$goGetLastHeader = preg_replace("/,$/",'',(string) $goGetLastHeader);
			$goGetLastHeader2 = explode(",",$goGetLastHeader);
			$goGetLastCustomFiledsName = preg_replace("/,$/",'',(string) $goGetLastCustomFiledsName);
		...
		while (($data = fgetcsv($handle, 1000, $default_delimiter, escape: '\\')) !== FALSE) {
			...
			if(!empty($phone_code_override)) {
				$phone_code = preg_replace($field_regx, "", (string) $phone_code_override);
			}
			else {
			...
			if(!empty($lead_mapping)){
				...
				foreach($standard_array as $l => $map_data){
					...
					if($map_data !== "" && $map_data !== "."){
						$mappedValue = $col[$map_data] ?? '';
						// one by one sort through columns to overwrite lead mapping data
						if($l == "Phone")
							$phone_number = $mappedValue;
						if($l == "VendorLeadCode")
							$vendor_lead_code = $mappedValue;
						if($l == "PhoneCode"){
							if(!empty($phone_code_override))
								$phone_code = $phone_code_override;
							else
								$phone_code = $mappedValue;
						}if($l == "Title")
							$title = $mappedValue;
						if($l == "FirstName")
							$first_name = $mappedValue;
						if($l == "MiddleInitial")
							$middle_initial = $mappedValue;
						if($l == "LastName")
							$last_name = $mappedValue;
						if($l == "Address1")
							$address1 = $mappedValue;
						if($l == "Address2")
							$address2 = $mappedValue;
						if($l == "Address3")
							$address3 = $mappedValue;
						if($l == "City")
							$city = $mappedValue;
						if($l == "State")
							$state = $mappedValue;
						if($l == "Province")
							$province = $mappedValue;
						if($l == "PostalCode")
							$postal_code = $mappedValue;
						if($l == "CountryCode")
							$country_code = $mappedValue;
						if($l == "Gender")
							$gender = $mappedValue;
						if($l == "DateOfBirth")
							$date_of_birth = $mappedValue;
						if($l == "AltPhone")
							$alt_phone = $mappedValue;
						if($l == "Email")
							$email = $mappedValue;
						if($l == "SecurityPhrase")
							$security_phrase = $mappedValue;
						if($l == "Comments")
							$comments = $mappedValue;
					}// end if
				}// end loop
			} // END OF LEAD MAPPING
				...
				if($goGetCheckcustomFieldNamesCorrect == "error" && empty($lead_mapping)) {
					...
					if($countCheck1 < 1 && $countCheck2 < 1){
						...
						$insertData = [
							...
							'title' => $title,
							'first_name' => mb_convert_encoding((string) $first_name, 'UTF-8', 'ISO-8859-1'),
							'middle_initial' => mb_convert_encoding((string) $middle_initial, 'UTF-8', 'ISO-8859-1'),
							'last_name' => mb_convert_encoding((string) $last_name, 'UTF-8', 'ISO-8859-1'),
							'address1' => $address1,
							'address2' => $address2,
							'address3' => $address3,
							'city' => $city,
						...
						if(!empty($lead_mapping) && !empty($custom_array)){ // LEAD MAPPING CUSTOMIZATION
							...
							foreach($custom_array as $custom_key => $map_data){
								$custom_key = (string) $custom_key;
								if (!preg_match('/^[A-Za-z0-9_]+$/', $custom_key) || !is_numeric($map_data)) {
									continue;
								}
								$map_index = (int) $map_data;
								$goCustomValues = preg_replace($field_regx, "", (string) ($col[$map_index] ?? ''));
								//8626
								if(is_null($goCustomValues)  || $goCustomValues === "")
									$goCustomValues = "";
								else
									$goCustomValues = str_replace($default_delimiter, "", $goCustomValues);
								$goCustomValues = $astDB->escape((string) $goCustomValues);
								$goCustomKeyData[] = "`$custom_key`";
								$goCustomValuesData[] = "'$goCustomValues'";
								$goCustomUpdateData[] = "`$custom_key`='$goCustomValues'";
							}
							...
							if($countResultrsltgoShowCustomFields > 1) {
								...
								for($ax=0; $ax < $totalExplode; $ax++) {
									$goHeaderOfCustomFields = (string) ($goGetLastCustomFiledsName2[$ax] ?? ''); #get the header name of the custom fields
									$header_index = $goGetLastHeader2[$ax] ?? null;
									if (!preg_match('/^[A-Za-z0-9_]+$/', $goHeaderOfCustomFields) || !is_numeric($header_index)) {
										continue;
									}
									$goCustomValues = preg_replace($field_regx, "", (string) ($col[(int) $header_index] ?? '')); #get the values of the custom fields
									//8626
									if(is_null($goCustomValues)  || $goCustomValues === "")
										$goCustomValues = "";
									else
									...
									$goCustomValues = $astDB->escape((string) $goCustomValues);
									$goCustomValuesData[] = "'$goCustomValues'";
									$goCustomUpdateData[] = "`$goHeaderOfCustomFields`='$goCustomValues'";
								}
								$goHeaderOfCustomFields = implode(",", array_map(static function ($fieldName) { return "`" . str_replace("`", "``", (string) $fieldName) . "`"; }, array_filter($goGetLastCustomFiledsName2, static function ($fieldName) { return preg_match('/^[A-Za-z0-9_]+$/', (string) $fieldName); })));
								$goCustomValues = implode(",", $goCustomValuesData);
								$goCustomUpdate = implode(", ",  $goCustomUpdateData);
				...
				if($goGetCheckcustomFieldNamesCorrect == "error" && empty($lead_mapping)) {
					...
					if($countResult > 0) {
						...
						if($countCheckCampPhoneList < 1) {
							...
							# end set query for custom fields
							$goCountInsertedLeads++;
															$apiresults = ["result" => "success", "message" => "$goCountInsertedLeads"];
						}// end of IF
						else{
							$duplicates++;
						}
...
//								$goQueryCustomFields .= "INSERT INTO `custom_$theList`(lead_id, $goHeaderOfCustomFields) VALUES('$goLastInsertedLeadIDNODUP', '$goCustomValues') ON DUPLICATE KEY UPDATE $goHeaderOfCustomFields='$goCustomValues';";
					...
					$goCountInsertedLeads++;
					$apiresults = ["result" => "success", "message" => "$goCountInsertedLeads", "alex_data" => $alex];
		...
		if($goCountInsertedLeads > 0 && $duplicates < 1) {
			$apiresults = ["result" => "success", "message" => "Total Uploaded Leads: $goCountInsertedLeads" , "alex_data" => $alex];
		}elseif($goCountInsertedLeads > 0 && $duplicates > 0){
			$apiresults = ["result" => "success", "message" => "Uploaded:$goCountInsertedLeads    \nDuplicates:$duplicates"];
		} elseif($goGetCheckcustomFieldNamesCorrect == "error"){
			$apiresults = ["result" => "error" , "message" => "Error: Lead File Not Compatible with List. \nIncompatible File Headers \n\nPlease use \"Lead Mapping\" function instead"];
		}elseif($duplicates > 0){
			$apiresults = ["result" => "error" , "message" => "Duplicates Found : $duplicates"];
		}else {
			$apiresults = ["result" => "error", "message" => "$goCountInsertedLeads", "duplicates" => $duplicates, "alex_data" => $alex];
		}
	...
	function goGetCampaignList($link, $goCampaignID) {
		...
		foreach ((is_array($rsltgoCheckCamp) ? $rsltgoCheckCamp : []) as $fresultsDup) {
			if (($fresultsDup['list_id'] ?? '') !== '') {
	...
	function goCheckCustomFieldsName($link, $goCClistID, $gocustomFieldsCSV) {
		$goCClistID = preg_replace('/\D+/', '', (string) $goCClistID);
		$requestedFields = array_values(array_filter(array_map('trim', explode(',', (string) $gocustomFieldsCSV)), static function ($fieldName) {
			return $fieldName !== '' && preg_match('/^[A-Za-z0-9_]+$/', $fieldName);
		}));

		if ($goCClistID === '' || count($requestedFields) === 0) {
			return "error";
		}

		$tableName = "custom_$goCClistID";
		$tableCheck = $link->rawQuery("SHOW TABLES LIKE '$tableName'");
		if (!is_array($tableCheck) || count($tableCheck) < 1) {
			return "error";
		}

		$columns = $link->rawQuery("SHOW COLUMNS FROM `$tableName`");
		$availableFields = [];
		foreach ((is_array($columns) ? $columns : []) as $column) {
			$fieldName = is_array($column) ? (string) ($column['Field'] ?? '') : '';
			if ($fieldName !== '') {

...
/** @var string $log_ip */
	...
	} elseif (empty($call_time_id) || strlen((string) $call_time_id) < 3) {
		$apiresults 									= [
			"result" 										=> "Error: Set a value for Call Time ID not less than 3 characters."
		];
	} elseif (preg_match('/[\'^Â£$%&*()}{@#~?><>,|=_+Â-]/',(string) $call_time_name) || empty($call_time_name)) {
		$apiresults 									= [
		...
		if ($goapiaccess > 0 && $userlevel > 7) {    
			...
			if ($countResult > 0) {
				...
				if ($astDB->count > 0) {
					...
					if (!$updateQuery) {
						$apiresults 					= [
							"result" 						=> "Error: Edit failed, check your details"
						];
					} else {
						$log_id 						= log_action($goDB, 'MODIFY', $log_user, $log_ip, "Updated Call Time ID: $call_time_id", $log_group, $astDB->getLastQuery());
						$apiresults 					= [
							"result" 						=> "success"
						];
					}
					...
					$apiresults 						= [
						"result" 							=> "Error: Edit failed, Call Time doesn't exist!"
					];
				}
			} else {
				$apiresults 							= [

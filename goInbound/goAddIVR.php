...
*/
    ...
    if(empty($menu_id) || strlen($menu_id) < 4) {
		...
		// Admin logs
                if($query){
                    $log_id = log_action($goDB, 'ADD', $log_user, $ip_address, "Added New IVR $menu_id", $log_group, $queryAddIVR);
                    $apiresults = array("result" => "success", "query" => $queryAddIVR);
                }else{
                    $apiresults = array("result" => "Error: Query Failed");
                }
			}

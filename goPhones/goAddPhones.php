...
/** @var string $log_ip */
	...
	} elseif (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $orig_extension)) {
		$apiresults 									= [
			"result" 										=> "Error: Special characters found in extension"
		];
	} elseif (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $server_ip) || $server_ip == null) {
		$apiresults 									= [
	...
	} elseif (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $dialplan_number) || $dialplan_number == null) {
		$apiresults 									= [
			"result" 										=> "Error: Special characters found in dialplan_number and must not be null"
		];
	} elseif (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $voicemail_id) || $voicemail_id == null) {
		$apiresults 									= [

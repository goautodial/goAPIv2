<?php
    ...
    function go_get_groupid($goUser){
        ...
        $rsltv = mysql_query($query_userv, $link);
        $check_resultv = mysql_num_rows($rsltv);
    
        if ($check_resultv > 0) {
            $rowc=mysql_fetch_assoc($rsltv);
    ...
    function checkIfTenant($groupId){
        ...
        $rslt_tenant = mysql_query($query_tenant, $linkgo);
        $check_result_tenant = mysql_num_rows($rslt_tenant);
    ...
    function go_getall_allowed_users($groupId) {
        ...
        $fresults=mysql_fetch_array($rsltv);
        $callfunc = go_total_agents_callv($groupId);
        $v = $callfunc - 1;
        $allowed_users='';
        $i=0;
        
        while($info = mysql_fetch_array( $rsltv )) {
            $users = $info['userg'];
                if ($i==$v) {
                      $allowed_users .= "'" . $users. "'";
                } else {
    ...
    function go_total_agents_callv($groupId) {
      ...
      function go_getall_allowed_campaigns($groupId)
      {
            ...
            $resultsu = mysql_query($query,$link);

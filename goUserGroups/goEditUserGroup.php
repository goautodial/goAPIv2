...
########################
        ...
        if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $group_name)){
                ...
                $rsltv = mysqli_query($link, $query);
					...
					//$datagroup_level = $fresults['group_level'];
					$dataforced_timeclock_login = $fresults['forced_timeclock_login'];
					$datashift_enforcement = $fresults['shift_enforcement'];
						...
						if($var=="group_level"){
							$group_level = "$val";
                                ...
                                */
                                ...
                                $query = "UPDATE vicidial_user_groups SET group_name='$group_name', forced_timeclock_login='$forced_timeclock_login', shift_enforcement='$shift_enforcement' WHERE user_group='$datauser_group';";
				$rsltvQuery = mysqli_query($link, $query);

				$queryGL = "UPDATE user_access_group SET group_level = '$group_level' WHERE user_group='$datauser_group';";

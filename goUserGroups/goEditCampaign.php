	...
	if($campaign_id == null) { 
		...
		if(!in_array($active,$defActive) && $active != null) { 
			...
			if(!in_array($dial_method,$defDialMethod) && $dial_method != null) { 
				...
				if($dataCampID != null) {	
					$updateQuery = "UPDATE vicidial_campaigns SET campaign_name='".mysqli_real_escape_string($uCampaignName)."', dial_method='$uDialMethod', active='$uActive',auto_dial_level '$auto_dial_level', local_call_time = '$local_call_time', campaign_recording = '$campaign_recording'   WHERE campaign_id='$dataCampID' LIMIT 1;";
					//echo $updateQuery;

...
function array_to_xml( $data, &$xml_data ) {
    foreach( $data as $key => $value ) {
        if( is_array($value) ) {
            ...
            $getKey = get_node_type($xml_data->getName());
            if ( is_numeric($key) ) {
                $newKey = $getKey ?? 'item'; // XML Naming Rules: Names cannot start with a number or punctuation character
...
function get_node_type( $type ) {
    return match ($type) {
        "campaigns", "allowed_campaigns" => 'campaign',
        "statuses" => 'status',
        "pause_codes" => 'code',
        "xfer_groups", "xfer_group_names", "inbound_groups", "email_groups", "phone_groups" => 'group',
        "all_callbacks", "live_callbacks", "today_callbacks" => 'callback',
        default => null,
    };
}

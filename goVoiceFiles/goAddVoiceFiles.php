...
*/
	...
	if (empty($log_user) || is_null($log_user)) {
		...
		if ( $stage == "upload") {
			//$audiofile
			$explodefile 						= explode(".",strtolower((string) $audiofile_orig));
			$prefix 							= ( checkIfTenant($log_group, $goDB) ) ? "go_".$log_group."_" : "go_";

			if ( preg_match("/\.(wav|mp3)$/i",(string) $audiofile_orig) ) {
				$audiofile_dir 					= preg_replace("/ /",'\ ',(string) $audiofile_dir);
				$audiofile_dir 					= preg_replace("/@/",'\@',$audiofile_dir);
				$audiofile_name 				= preg_replace("/ /",'',"$prefix".$audiofile_name);
				$audiofile_name 				= preg_replace("/@/",'',(string) $audiofile_name);			
				$audio_filesize 				= formatSizeUnits($audiofile_size);			
				$encoded_audio 					= base64_encode((string) $audiofile_name);
				...
				if ( $count_sounds < 1 ) {
					...
					if ( file_exists("$path_sounds/$audiofile_name") ) {
						...
						if ( $exec_sounds ) {
							...
							$rsltUpdate 		= $astDB->update( 'servers', ['sounds_update' => 'Y'] );
						} else {

						...
						if(!in_array($access_voicemails,$defaccess_voicemails) && $access_voicemails != null) {
							...
							if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $tenant_id)){
								...
								if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $tenant_name)){
									...
									if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $admin) || $admin  == null){
										$apiresults = ["result" => "Error: Special characters found in admin and must not be empty"];
									} else {
										if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $pass) || $pass == null){
											...
											if($countResultCheck > 0) {
												$apiresults = ["result" => "Error: Tenant ID already exist"];
											} else {
												...
												if($countResult > 0 && $countResultOne > 0 ) {
													...
													$apiresults = ["result" => "Error: Failed to add Tenant."];
												}
											}
										}
									}

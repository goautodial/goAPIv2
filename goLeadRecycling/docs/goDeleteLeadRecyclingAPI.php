<?php
 ...
 ####################################################
 	...
 	$url = "https://gadcs.goautodial.com/goAPI/goLeadRecycling/goAPI.php"; #URL to GoAutoDial API. (required)
        ...
        $postfields["status"] = $_GET['status']; #Desired Lead Filter id. (required)
        $postfields["leadRecCampID"] = $_GET['leadRecCampID']; #Desired Lead Filter id. (required)
...
//	print_r($data);
	...
	if ($output->result=="success") {
	   # Result was OK!
		echo "Delete Success ".$_GET['status'];	
	 } else {
	   # An error occured
	   	echo $output->result;
	}

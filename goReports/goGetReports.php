            ...
            //Start Report
				...
				if ($pageTitle=='stats'){
					...
					if ($return['request']=='daily') {
						...
						$query = mysqli_query($link, "select phone_number from vicidial_log vl where length_in_sec>'0' and date_format(call_date, '%Y-%m-%d %H:%i:%s') between '$fromDate' and '$toDate' and campaign_id = '$campaignID'  $ul $TunionSQL");
						...
						$query = mysqli_query($link, "select status, sum(ccount) as ccount from (select status,count(*) as ccount from vicidial_log vl where length_in_sec>'0' and campaign_id = '$campaignID' and MONTH(call_date) between MONTH('$fromDate') and MONTH('$toDate') $ul group by status $DunionSQL) t group by status;");
						$total_status = mysqli_num_rows($query);
						
						$query = mysqli_query($link, "select status, sum(ccount) as ccount from (select status,count(*) as ccount from vicidial_log vl where length_in_sec>'0' and campaign_id = '$campaignID' and date_format(call_date, '%Y-%m-%d %H:%i:%s') between '$fromDate' and '$toDate' $ul group by status $DunionSQL) t group by status;");
						while($row = mysqli_fetch_array($query, MYSQLI_ASSOC)){
							$status[] = $row['status'];
							$ccount[] = $row['ccount'];

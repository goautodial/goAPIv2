...
/** @var string $VARDB_database */
	...
	// new
	// ----
	$campaignFilterValue = is_array($campaigns) ? implode(',', $campaigns) : (string) $campaigns;
	$inboundFilterValue = is_array($inbounds) ? implode(',', $inbounds) : (string) $inbounds;
	$listFilterValue = is_array($lists) ? implode(',', $lists) : (string) $lists;
	$campFilter = ($campaignFilterValue !== '') ? "Campaign(s): $campaignFilterValue" : "";
	$inbFilter = ($inboundFilterValue !== '') ? "Inbound Groups(s): $inboundFilterValue" : "";
	$listFilter = ($listFilterValue !== '') ? "List(s): $listFilterValue" : "";

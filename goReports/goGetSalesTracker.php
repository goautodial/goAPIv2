...
/** @var string $VARDB_database */
	...
	if (empty($log_user) || is_null($log_user)) {
		...
		$sstatuses = is_array($sstatuses) ? implode("','", $sstatuses) : (string) $sstatuses;
		...
		$rawQstatus2 = "select status from vicidial_campaign_statuses where sale='Y' and campaign_id IN (".$imploded_camp.");";
		...
		$cstatuses = is_array($cstatuses) ? implode("','", $cstatuses) : (string) $cstatuses;
		...
		if (strtolower((string) $request) === 'outbound') {
			$outbound_query 				= "
				...
				AND vlo.campaign_id IN ($imploded_camp)

...
error_reporting(E_ALL);
	...
	if(isset($_REQUEST['call_export_report']))
	...
	//if(isset($_REQUEST['sales_tracker_request']))
	$sales_tracker_request = "outbound";
	//$sales_tracker_request = $_REQUEST['sales_tracker_request'];
	
	//$returns = $pageTitle.' / '.$fromDate.' / '.$toDate.' / '.$campaignID;
    
    $goReportsReturn = go_get_reports($pageTitle,'2016-07-01 00:00:00','2016-07-08 23:59:59','82247255','daily','admin','usergroup',$link, $sales_tracker_request);
	...
	function go_get_reports($pageTitle, $fromDate, $toDate, $campaignID, $request, $userID, $userGroup, $link, $sales_tracker_request){
...
//			 $mysqli_query($link, cache_on();
            ...
            while($Qstatus = mysqli_fetch_array($query2, MYSQLI_ASSOC)){
				...
				if(isset($cstatuses) && $cstatuses != NULL)
			...
			if ($pageTitle=='stats'){
				...
				if ($return['request']=='daily') {
					...
					$return['call_time'] = go_get_calltimes($campaignID, $link);
				...
				$apiresults = array("data_calls" => $data_calls, "data_status" => $data_status, "data_agents" => $data_agents, "total_calls" => $total_calls, "total_leads" => $total_leads, "total_new" => $total_new, "total_status" => $total_status);
	...
	//die($timeclock_ct);
			...
			if ($pageTitle == "sales_agent") {
				...
				if($numO > 0) {
					while($row = mysqli_fetch_array($query)) {
						...
						$TOPsorted_output .= "</tr>";
						$total_out_sales = $total_sales + $row['sale'];
					}
				}
				...
				if ($numI) {
					while($row = mysqli_fetch_array($query)){
						...
						$BOTsorted_output .= "</tr>";
						$total_in_sales = $total_sales + $row['sale'];
					}
				}
			...
			if ($pageTitle == "sales_tracker") {
				...
				if ($sales_tracker_request == 'outbound') {
				...
				if ($sales_tracker_request == 'inbound') {
					$query = mysqli_query($link, "SELECT closer_campaigns FROM vicidial_campaigns WHERE campaign_id='$campaignID' ORDER BY campaign_id");
					$row = mysqli_fetch_array($query);
					$closer_camp_array=explode(" ",$row['closer_campaigns']);
					$num=count($closer_camp_array);
			...
			if ($pageTitle=="dashboard") {
				...
				if (count($agent)>0)
				{
					...
					foreach ($query->result() as $i => $user_info)
					{
						...
						$TOPsorted_output .= "<tr><td nowrap style=\"border-top:#D0D0D0 dashed 1px;\"><div align=\"center\" class=\"style3 toolTip\" title=\"".$user_info->user."\">&nbsp;<strong>".$user_info->full_name."</strong></td>";
						
						$t = 0;
						foreach ($Dstatus as $s)
						{
							$call_cnt = ($agent[$user_info->user][$s]>0) ? $agent[$user_info->user][$s] : 0;
							$TOPsorted_output .= "<td nowrap style=\"border-top:#D0D0D0 dashed 1px;\"><div align=\"center\" class=\"style4\">&nbsp;".$call_cnt."</td>";
							$sub_total[$user_info->user] = $sub_total[$user_info->user] + $agent[$user_info->user][$s];
							$t++;
						}
	
						$TOPsorted_output .= "<td nowrap style=\"border-top:#D0D0D0 dashed 1px;\"><div align=\"center\" class=\"style3\">&nbsp;".$sub_total[$user_info->user]."</td></tr>";

		...
		*/
		...
		if ($astDB->count >0) {
			foreach ($queryx as $row) {
				...
				$calledCount = (int) ($row['called_count'] ?? 0);
				$rowCount = (int) ($row['count'] ?? 0);
				$all_called_count[$calledCount] = ($all_called_count[$calledCount] ?? 0) + $rowCount;

				if ( strlen((string) ($status[$sts] ?? '')) < 1 || ($status[$sts] ?? '') != $row['status'] ) {
					if ($first_row) {
						$first_row = 0;
					} else{
						$sts++;
					...
					$status_called_first[$sts] 	= $calledCount;
				...
				$leads_in_sts[$sts] = ($leads_in_sts[$sts] ?? 0) + $rowCount;
				$status_called_last[$sts] 		= $calledCount;
		...
		while ($statuses_called_to_print > $sts) {
			...
			$TOPsorted_output .= "
				<tr>
					...
					<td nowrap> ".($statuses_list[$Pstatus] ?? $Pstatus)." </td>
				";
			...
			$TOPsorted_output .= "<td nowrap> ".($leads_in_sts[$sts] ?? 0)." </td></tr>\n\n";
			$sts++;
		}
		...
		while ($first <= $all_called_last) {
			//if ($all_called_count[$first] == $first) {
				$all_called_count_data = $all_called_count[$first] ?? 0;
				$TOPsorted_output 			.= "
	...
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		if ($astDB->count > 0) {
			foreach ($numBot as $rowBot) {
				...
				$called_leadCount 			= $rowC['Clead_count'] ?? 0;
				...
				$notcalled_leadCount = 0;
				foreach($fetchCalled as $rowNC)
				$notcalled_leadCount 		= $rowNC['NClead_count'] ?? 0;

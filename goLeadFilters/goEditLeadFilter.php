...
/** @var string $log_ip */
    ...
    if($lead_filter_id == null) { 
        ...
        if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', (string) $lead_filter_name)){
            $apiresults = ["result" => "Error: Special characters found in lead filter name"];
        } else {
			if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $lead_filr_comments)){
                $apiresults = ["result" => "Error: Special characters found in lead filter comments"];
			} else {
